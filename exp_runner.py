...
class Runner:
    def __init__(self, conf_path, data_dir, mode='train', case='CASE_NAME', is_continue=False, n_latent_codes=None):
        ...
        # Per-view latent codes
        if n_latent_codes is None:
            n_latent_codes = self.dataset.n_images
        self.latent_codes = torch.nn.Parameter(torch.randn(
            n_latent_codes, self.conf['model.rendering_network.d_latent_code']).float().cuda() * 0.01)
        self.latent_codes.requires_grad_(True)
        params_to_train += [self.latent_codes, ]
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            idx = image_perm[self.iter_step % len(image_perm)]
            data = self.dataset.gen_random_rays_at(idx, self.batch_size)

            rays_o, rays_d, true_rgb, mask = data[:,
                                                  :3], data[:, 3: 6], data[:, 6: 9], data[:, 9: 10]
            near, far = self.dataset.near_far_from_sphere(rays_o, rays_d)

            # print('latent_code: ',
            #       self.latent_codes[:4, :4].data.clone().detach().cpu().numpy())
            ...
            render_out = self.renderer.render(self.latent_codes[idx], rays_o, rays_d, near, far,
    ...
    def load_checkpoint(self, checkpoint_name):
        ...
        checkpoint = torch.load(os.path.join(
            self.base_exp_dir, 'latent_codes', checkpoint_name), map_location=self.device)
        self.latent_codes.data.copy_(checkpoint)
    ...
    def save_checkpoint(self):
        ...
        os.makedirs(os.path.join(self.base_exp_dir,
                    'latent_codes'), exist_ok=True)
        torch.save(self.latent_codes.data, os.path.join(self.base_exp_dir,
                   'latent_codes', 'ckpt_{:0>6d}.pth'.format(self.iter_step)))

    def validate_image(self, idx=-1, resolution_level=-1, no_write=False, latent_code_idx=None):
        if idx < 0:
        ...
        for rays_o_batch, rays_d_batch in zip(rays_o, rays_d):
            ...
            render_out = self.renderer.render(self.latent_codes[latent_code_idx],
                                              rays_o_batch,
                                              rays_d_batch,
                                              near,
                                              far,
        ...
        if len(out_rgb_fine) > 0:
            img_fine = (np.concatenate(out_rgb_fine, axis=0).reshape(
                [H, W, 3, -1]) * 256).clip(0, 255).astype(np.uint8)

        normal_img = None
        if len(out_normal_fine) > 0:
            normal_img = (np.concatenate(out_normal_fine, axis=0).reshape(
                [H, W, 3, -1]) * 128 + 128).clip(0, 255).astype(np.uint8)
        ...
        if no_write:
            # ic(img_fine.shape, normal_img.shape)
            return img_fine[..., 0], normal_img[..., 0]
    ...
    def validate_mesh(self, world_space=False, resolution=64, threshold=0.0, out_fpath=None):
        ...
        vertex_colors = []
        for pts in torch.split(torch.from_numpy(vertices.astype(np.float32)).reshape(-1, 3).cuda(), 16384, dim=0):
            feature_vector = self.renderer.sdf_network(pts)[:, 1:]
            gradients = self.renderer.sdf_network.gradient(pts).squeeze()
            color = self.renderer.color_network(self.latent_codes[0], pts, gradients, gradients,
                                                feature_vector)
            vertex_colors.append(color.detach().cpu())
        vertex_colors = torch.cat(vertex_colors, dim=0).numpy()
        vertex_colors = np.clip((vertex_colors * 255.),
        ...
        if out_fpath is None:
            os.makedirs(os.path.join(
                self.base_exp_dir, 'meshes'), exist_ok=True)
            out_fpath = os.path.join(
                self.base_exp_dir, 'meshes', '{:0>8d}.obj'.format(self.iter_step))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--render_split_tag', type=str, default='test')
    parser.add_argument('--n_latent_codes', type=int, default=None)

    args = parser.parse_args()

    torch.cuda.set_device(args.gpu)
    runner = Runner(args.conf, args.data_dir, args.mode,
                    args.case, args.is_continue, args.n_latent_codes)

    if args.mode == 'train':
        runner.train()
    elif args.mode == 'validate_mesh':
        out_dir = os.path.join(runner.base_exp_dir, 'render_only_{}_{:0>8d}'.format(
            args.render_split_tag, runner.iter_step))
        os.makedirs(out_dir, exist_ok=True)

        runner.validate_mesh(world_space=False, resolution=512,
                             threshold=args.mcube_threshold, out_fpath=os.path.join(out_dir, 'mesh_norm.obj'))

        for idx in range(runner.dataset.images.shape[0]):
            img_name = os.path.basename(runner.dataset.images_lis[idx])[
                :-4] + '.png'
            # if os.path.isfile(os.path.join(out_dir, img_name)):
            #    ic('skipping: {}'.format(img_name))
            #    continue

            # if int(img_name[:-4]) < 100:
            #    ic('skipping: {}'.format(img_name))
            #    continue

            print('rendering {}/{} image'.format(idx,
                  runner.dataset.images.shape[0]))
            img_pred, img_normal = runner.validate_image(
                idx, resolution_level=1, no_write=True, latent_code_idx=0)
            cv.imwrite(os.path.join(out_dir, img_name), img_pred)
            cv.imwrite(os.path.join(
                out_dir, img_name[:-4]+'_normal.png'), img_normal[..., ::-1])
